...
前端每次操作（切换会话/发送消息） 
  → GET /sessions/{sessionId}/messages
    → 后端 SELECT * FROM chat_messages WHERE session_id = ?
      → 返回全部消息（含完整树的所有节点）
        → 前端构建 messageMap → messageTree → 筛选 activePath
```

**问题**：
1. **重复 DB 查询**：前端切回一个已有会话时，后端需要重新 `SELECT` 该会话的所有消息
2. **无缓存层**：`chat_messages` 表没有读缓存，每次请求直接穿透到 PostgreSQL
3. **Chat 树弹窗也需要全量消息**：前端 `userQuestionRoots` 依赖 `messageTree`，目前必须等待完整消息列表加载

**不应该做的**：为每个请求新增 DB 查询（如查询 active path 专用 API），这违反"一次拉取整棵树"的设计初衷。
...
│  Type: Hash                                           │
│  Fields: {msgId}: {序列化的 ChatMessageVO JSON}       │
│  TTL:  30 min (可配置)                                │
│                                                       │
│  用途：缓存每个会话的全部消息，供前端一次拉取后构造树     │
│                                                       │
└───────────────────────────────────────────────────────┘
```

**为什么选用 Hash 而非 String/List？**

| 数据结构 | 适用性 | 说明 |
|---------|--------|------|
| **Hash** ✅ | 最佳 | Field = msgId，Value = 序列化消息。支持单条插入/删除（新消息到达时只写一个 field），支持批量读取（`HGETALL`），内存效率高 |
| String (JSON数组) | 次优 | 需要完整覆盖写，新消息到达时需反序列化→追加→序列化→写回，并发风险 |
| List | 不适用 | 消息顺序重要但 List 不支持按 msgId 操作 |
...
| 操作 | 缓存动作 |
|------|---------|
| `POST /rag/chat`（发送新消息） | `HSET` 追加 2 条新消息（user + assistant）；刷新 TTL |
| `DELETE /sessions/{id}`（删除会话） | `DEL session:msgs:{id}` |
| `DELETE /messages/{id}/subtree`（删除子树） | `HDEL` 删除子树中所有 msgId 的 field |
| 服务重启 | 自动过期（TTL 兜底），无需手动清理 |

**activePath 额外缓存（可选 Layer 2）**：

如果前端展示 active path 消息的响应时间仍然不够快：

```
Key: session:path:{sessionId}:{activeLeafId}
Type: String (JSON 数组)
存储：从根到 activeLeafId 的消息列表（已过滤）
TTL: 10 min
```

写入时机：前端第一次请求该 active path 时，计算后写入。activeLeafId 变化时失效。
...
rag:
  cache:
    session-messages-ttl: 1800  # 会话消息缓存 TTL（秒）
```
...
| `application.yaml` | 添加 Redis 连接配置 + `rag.cache.session-messages-ttl` |
| 新增 `config/RedisConfig.java` | RedisTemplate 序列化配置（Jackson2JsonRedisSerializer） |
| 新增 `service/impl/ChatMessageCacheService.java` | 封装缓存读写逻辑（Hash CRUD + TTL 管理） |
| `ChatServiceImpl.java` | `chat()` 方法中插入新消息后调用 `HSET`；`backtrackHistory` 直接从缓存读取 |
| `ChatController.java` | `getMessages()` 先查缓存再查 DB |
| `ChatSessionServiceImpl.java` | `deleteSession()` 中调用 `DEL` 清除缓存 |

### 9.5 实施优先级

| 优先级 | 说明 |
|--------|------|
| 🟡 **中** | 当前会话消息量通常较小（几十条），DB 查询性能尚可。建议在以下时机启动：1) 会话消息量增长导致前端感知到明显延迟时；2) 准备上线多用户场景时；3) 开发流程有空档时作为"质量优化"项 |

### 9.6 补充思考：缓存粒度权衡

| 方案 | 缓存位置 | 优点 | 缺点 |
|------|---------|------|------|
| Redis Hash (推荐) | 服务端 | 多实例共享，自动过期，内存可控 | 需要 Redis 实例 |
| Vuex / Pinia | 前端 | 零延迟，无需后端改动 | 页面刷新即丢失，多标签页不共享 |
| `Map<Integer, List>` in Service | 后端内存 | 实现最简单 | 无过期机制会导致 OOM，多实例不共享 |

**最终建议**：Redis Hash 缓存 + 前端 component `data()` 作为"当前会话的快照"（切换会话时清空），形成两层读优化：

```
前端 data 快照 (切换会话时加载) 
  → 命中：直接渲染，0 网络开销
  → 未命中：GET /sessions/{id}/messages
    → Redis Hash (TTL 30min)
      → PostgreSQL (持久化)
```
