 ...
 * study_plan 工作流类型化接口
 * <p>
 * 主工作流入口，输入 @V 参数从 Tool 层传入的初始 Map 中读取，
 * 子 Agent 在执行过程中向 AgenticScope 写入 plan_json / exam_json / clarification 等中间结果。
 * output() 函数负责解析 JSON 并持久化，返回 {@link StudyPlanWorkflowResult}。
 */
