...
/**
 * 知识收集阶段工作流编排服务。
 * 作为顶层 {@link StudyPlanner} 两阶段编排的第一阶段，负责：
 *   条件触发澄清（needs_clarification → 阻塞等待用户回复，仅一次）
 *   调用 {@link KnowledgeCollectionAgent} 自主搜索收集素材，写入 AgenticScope 的 "materials"
 * 编排顺序：clarifyConditional → collector。先澄清再收集，因为澄清后的补充信息可能引导新的搜索方向。
 * 阶段完成后，scope 中的 materials / clarification 供第二阶段{@link ContentGenerationStage} 使用。
 */
@Service
@Slf4j
@RequiredArgsConstructor
public class KnowledgeCollectionStage {
    ...
    public UntypedAgent build(StepRecorder recorder, ChatModel chatModel, Integer sessionId) {
        ...
        KnowledgeCollectionAgent collector = AgenticServices
                .agentBuilder(KnowledgeCollectionAgent.class)
                .chatModel(chatModel)
